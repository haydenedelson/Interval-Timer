...
class Timer:
    def __init__(self):
        ...
        def get_interval_lengths(self, intervals_dict):
            ...
            while i <= self.intervals:                    
                ...
                length_in_seconds = convert_to_seconds(interval_length)
                
                intervals_dict[f"interval_{i}"] = length_in_seconds
                i += 1
        
        # Call functions to set timer attributes
    ...
    def print_intervals(self):
        """
        Print current interval lengths to facilitate editing/overwriting
        """
        for key, value in self.intervals_dict.items():
            print(f"{key}: {value}")
